...
class SubmissionWorker(appContext: Context, workerParams: WorkerParameters) : CoroutineWorker(appContext, workerParams) {
    ...
    companion object {
        ...
        fun resetUnauthorizedCounter(context: Context) {
            PreferenceManager.getDefaultSharedPreferences(context)
                .edit {
                    putInt(PREF_CONSECUTIVE_UNAUTHORIZED, 0)
                        .putBoolean(PREF_SUBMISSIONS_BLOCKED, false)
                }
        }
    }
    ...
    override suspend fun doWork(): Result = withContext(Dispatchers.IO) {
        ...
        try {
                ...
                for (report in reports) {
                    when (val result = submitSingleReport(report)) {
                        ...
                        is SubmissionResult.PermanentError -> {
                            // This block checks for consecutive reports returning HTTP Unauthorized errors. These can
                            // be caused by either the user's credentials being wrong, or by a rare formatting bug in
                            // the program. (I highly suspect all such bugs are gone at this point, but this provides a
                            // safeguard.) If only a small number fail in a row, we'll assume the issue is with a single
                            // report. Otherwise, we'll assume a credential issue and block submissions until the user
                            // fixes the issue.
                            if (result.errorCode == HttpURLConnection.HTTP_UNAUTHORIZED) {
                                val currentCount = preferences.getInt(PREF_CONSECUTIVE_UNAUTHORIZED, 0)
                                val newCount = currentCount + 1
                                preferences.edit { putInt(PREF_CONSECUTIVE_UNAUTHORIZED, newCount) }
            ...
            return@withContext when {
                processedAllReports -> Result.success(
                    workDataOf("submission_time" to System.currentTimeMillis())
                )
                else -> Result.retry()
            }
        } catch (e: Exception) {
    ...
    private fun notifyCredentialIssue(failureCount: Int) {
        ...
        val settingsPendingIntent = PendingIntent.getActivity(
            applicationContext, 0, settingsIntent, PendingIntent.FLAG_IMMUTABLE
        )
        ...
        val resetPendingIntent = PendingIntent.getBroadcast(
            applicationContext, 0, resetIntent, PendingIntent.FLAG_IMMUTABLE
        )

        // Build notification with multiple actions
        val builder = NotificationCompat.Builder(applicationContext, "com.calindora.follow.default")
            .setSmallIcon(R.drawable.ic_stat_notification)
            .setContentTitle("Authentication Problem Detected")
            .setContentText("$failureCount consecutive reports failed authentication. PLease check your device key and secret.")
            ...
            when (responseCode) {
                ...
                HttpURLConnection.HTTP_UNAUTHORIZED -> {
                    ...
                    if (errorMessage.contains("invalid signature", ignoreCase = true)) {
                        SubmissionResult.PermanentError(responseCode, "Invalid signature for this report")
                    } else {
